...
class DFA(fa.FA):
    ...
    def _validate_input_yield_was_final_state(self, input_str):
        "UIS. This function help to identificate if an automaton went through a terminal state"
        current_state = self.initial_state
        flag = False
        yield current_state
        for i,input_symbol in enumerate(input_str):
            current_state = self._get_next_current_state(
                current_state, input_symbol)
            yield current_state
            if current_state in self.final_states:
                ...
                if i == len(input_str) - 1 and flag == False:
                    print ('the DFA did not go through a final state.')
    
    def _validate_input_return_was_final_state(self, input_str):
        """
        UIS.
        ...
        """
        validation_generator = self._validate_input_yield_was_final_state(input_str)
        flag = False
        for config in validation_generator:
            if config in self.final_states:
                flag = True
        if flag:
            return config
        
            
    def validate_input_when_was_final_state(self,input_str,step=False):
        """
        UIS.
        ...
        """
        if step:
            return self._validate_input_yield_was_final_state(input_str)
        else:
            return self._validate_input_return_was_final_state(input_str)
